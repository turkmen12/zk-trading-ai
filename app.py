...
st.markdown("<style>.stDataFrame {font-size: 10px;} .big-font {font-size:24px !important; font-weight: bold; color: #00ff00;}</style>", unsafe_allow_html=True)
...
if st.sidebar.button("🔍 تحليل السوق الآن"):
    with st.spinner('جاري جلب البيانات وتشغيل المحرك الذكي...'):
        try:
            ...
            if df.empty:
                st.error("⚠️ لم يتم العثور على بيانات. جرب رمزاً آخر أو إطاراً زمنياً أطول.")
                st.stop()
            ...
            # 2. استخراج القيم النهائية بأمان (تحويل صريح لـ float لتجنب خطأ Series)
            last_price = float(df['Close'].iloc[-1])
            last_rsi = float(df['RSI'].iloc[-1]) if not pd.isna(df['RSI'].iloc[-1]) else 50.0
            last_sma = float(df['SMA_50'].iloc[-1]) if not pd.isna(df['SMA_50'].iloc[-1]) else 0.0
            
            # 3. منطق التقييم الذكي (Smart Scoring)
            score = 50
            ...
            col4.markdown(f"<h3 style='color:{color}; text-align:center; margin-top:10px;'>{signal}</h3>", unsafe_allow_html=True)
